...
from pose_resnet_kinematic import pose_resnet101_heatmap
...
def main(args):
    ...
    loss_log_dir = osp.join('tensorboard', f'kinematic_loss_{datetime.now().strftime("%Y%m%d_%H%M")}')
    loss_writer = SummaryWriter(log_dir=loss_log_dir)
    error_log_dir = osp.join('tensorboard', f'kinematic_error_{datetime.now().strftime("%Y%m%d_%H%M")}')
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if epoch % 5 == 4:
            ...
            torch.save(model, f'trained_model_kinematic/model_after_{epoch + 1}_epochs.pkl')
        scheduler.step()

        # Draw the loss curve and validation indicator curve
        loss_writer.add_scalar('Loss', loss_avg.avg, epoch)
    print("\nSave model as model.pkl...")
    torch.save(model, 'trained_model_kinematic/model.pkl')
