...
from pose_resnet_self_attention import pose_resnet101_coordinate
...
def main(args):
    ...
    loss_log_dir = osp.join('tensorboard', f'attention_loss_{datetime.now().strftime("%Y%m%d_%H%M")}')
    loss_writer = SummaryWriter(log_dir=loss_log_dir)
    error_log_dir = osp.join('tensorboard', f'attention_error_{datetime.now().strftime("%Y%m%d_%H%M")}')
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if epoch % 5 == 4:
            ...
            torch.save(model, f'trained_model_attention/model_after_{epoch + 1}_epochs.pkl')
        scheduler.step()

        # Draw the loss curve and validation indicator curve
        loss_writer.add_scalar('Loss', loss_avg.avg, epoch)
    print("\nSave model as model.pkl...")
    torch.save(model, 'trained_model_attention/model.pkl')
